...
class PersonInfoDialog:
    ...
    def __init__(self, parent):
        ...
        # Giới tính
        tk.Label(form_frame, text="⚥ Giới tính:", font=("Arial", 10)).grid(
        ...
        tk.Radiobutton(
            gender_frame, text="Nam", variable=self.gender_var, value="Nam", font=("Arial", 10)
        ).pack(side=tk.LEFT, padx=5)
        tk.Radiobutton(
            gender_frame, text="Nữ", variable=self.gender_var, value="Nữ", font=("Arial", 10)
        ).pack(side=tk.LEFT, padx=5)
        self.gender_var.set("Nam")

        # Năm sinh
        tk.Label(form_frame, text="🎂 Năm sinh:", font=("Arial", 10)).grid(
        ...
        # Quê quán
        tk.Label(form_frame, text="🏡 Quê quán:", font=("Arial", 10)).grid(
        ...
        # Nơi sinh sống
        tk.Label(form_frame, text="📍 Nơi sinh sống:", font=("Arial", 10)).grid(
...
class FaceRecognitionGUI:
    ...
    def create_widgets(self):
        ...
        menu_title = tk.Label(
            left_panel, text="📋 CHỨC NĂNG", font=("Arial", 16, "bold"), bg="white", fg="#2c3e50"
        )
    ...
    def capture_from_webcam(self):
        ...
        title = tk.Label(
            self.right_panel, text="📸 Chụp ảnh từ Webcam", font=("Arial", 14, "bold"), bg="white"
        )
        title.pack(pady=10)
            ...
            if face_locations:
                ...
                if self.known_face_encodings:
                    face_distances = face_recognition.face_distance(
                        self.known_face_encodings, new_face_encoding
                    )
                ...
                if matched_person:
                    ...
                    if response:
                        ...
                        filename = os.path.join(
                            person_folder, f"{matched_person}_{existing_count + 1}.jpg"
                        )
                ...
                if person_info:
                    ...
                    temp_path = os.path.join(
                        "temp", f"temp_{datetime.now().strftime('%Y%m%d_%H%M%S')}.jpg"
                    )
    ...
    def stop_enrollment_recording(self):
        ...
        if matched_person:
            ...
            tk.Label(info_frame, text=info_text, font=("Arial", 11), justify=tk.LEFT).pack(pady=10)
            ...
            if choice == "save":
                ...
                existing_count = len(
                    [f for f in os.listdir(person_folder) if f.lower().endswith((".mp4", ".avi"))]
                )
                ...
                final_video_path = os.path.join(
                    person_folder, f"{matched_person}_video_{existing_count + 1}.mp4"
                )
                ...
                video_writer = cv2.VideoWriter(
                    temp_video_path, fourcc, 20.0, (frame_width, frame_height)
                )
        ...
        if person_info:
            ...
            for i, frame in enumerate(sampled_frames):
                temp_path = os.path.join(
                    "temp", f"temp_video_{i}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.jpg"
                )
            ...
            if success_count > 0:
                ...
                all_folders = [
                    ...
                    if os.path.isdir(os.path.join("faces", f))
                    and f.startswith(name.lower().replace(" ", "_"))
                ]
                if all_folders:
                    person_folder = os.path.join(
                        "faces", sorted(all_folders)[-1]
                    )  # Lấy folder mới nhất
        ...
        if file_path:
            ...
            if face_locations:
                ...
                if matched_person:
                    ...
                    tk.Label(info_frame, text=info_text, font=("Arial", 11), justify=tk.LEFT).pack(
                        pady=10
                    )
                    ...
                    if choice == "save":
                        ...
                        dest_path = os.path.join(
                            person_folder, f"{matched_person}_{existing_count + 1}{ext}"
                        )
    ...
    def recognize_from_file(self):
        ...
        if file_paths:
            ...
            threading.Thread(
                target=self.process_recognition_files, args=(file_paths,), daemon=True
            ).start()
    ...
    def process_recognition_files(self, file_paths):
        ...
        self.root.after(
            ...
            lambda: messagebox.showinfo(
                "Hoàn thành",
                f"✅ Đã nhận diện {total_faces} khuôn mặt!\nKết quả lưu trong thư mục 'recognized'",
            ),
        )
        self.root.after(
            0, lambda: self.update_status(f"Hoàn thành! Nhận diện {total_faces} khuôn mặt")
        )
    ...
    def organize_photos(self):
        ...
        if folder_path:
            ...
            threading.Thread(
                target=self.process_organize_photos, args=(folder_path,), daemon=True
            ).start()
    ...
    def process_organize_photos(self, folder_path):
        ...
        self.root.after(
            0, lambda: self.update_status(f"Đã tổ chức {organized_count + unknown_count} ảnh")
        )
    ...
    def manage_faces(self):
        ...
        title = tk.Label(
            self.right_panel, text="⚙️ Quản lý khuôn mặt", font=("Arial", 16, "bold"), bg="white"
        )
        title.pack(pady=20)
        ...
        self.face_listbox = tk.Listbox(
            list_frame, font=("Arial", 11), yscrollcommand=scrollbar.set, height=15
        )
    ...
    def refresh_face_list(self):
        ...
        if hasattr(self, "manage_info_label"):
            self.manage_info_label.config(text=f"Tổng số: {len(self.person_encodings)} người")
        ...
        if selection:
            ...
            if file_paths:
                ...
                old_files = [
                    f
                    for f in os.listdir("faces")
                    if os.path.isfile(os.path.join("faces", f))
                    and os.path.splitext(f)[0] == person_name
                ]
                for old_file in old_files:
                    shutil.move(
                        os.path.join("faces", old_file), os.path.join(person_folder, old_file)
                    )
                ...
                for file_path in file_paths:
                    ext = os.path.splitext(file_path)[1]
                    dest_path = os.path.join(
                        person_folder, f"{person_name}_{existing_count + added_count + 1}{ext}"
                    )
                    shutil.copy(file_path, dest_path)
                    added_count += 1
    ...
    def delete_face(self):
        ...
        if selection:
            ...
            confirm = messagebox.askyesno(
                "Xác nhận", f"Bạn có chắc muốn xóa '{name}' và tất cả ảnh của người này?"
            )
    ...
    def edit_person_info(self):
        ...
        if not current_info:
            messagebox.showerror(
                "Lỗi", f"Không tìm thấy thông tin của '{folder_name}' trong database!"
            )
            return
        ...
        tk.Label(form_frame, text="📁 Folder Name:", font=("Arial", 10, "bold")).grid(
            row=0, column=0, sticky="w", pady=5
        )
        ...
        tk.Radiobutton(
            gender_frame, text="Nam", variable=gender_var, value="Nam", font=("Arial", 10)
        ).pack(side=tk.LEFT, padx=5)
        tk.Radiobutton(
            gender_frame, text="Nữ", variable=gender_var, value="Nữ", font=("Arial", 10)
        ).pack(side=tk.LEFT, padx=5)
        tk.Radiobutton(
            gender_frame, text="Khác", variable=gender_var, value="Khác", font=("Arial", 10)
        ).pack(side=tk.LEFT, padx=5)

        # Năm sinh
        tk.Label(form_frame, text="🎂 Năm sinh:", font=("Arial", 10, "bold")).grid(
            row=3, column=0, sticky="w", pady=5
        )
        ...
        # Quê quán
        tk.Label(form_frame, text="🏠 Quê quán:", font=("Arial", 10, "bold")).grid(
            row=4, column=0, sticky="w", pady=5
        )
        ...
        # Nơi sinh sống
        tk.Label(form_frame, text="📍 Nơi sinh sống:", font=("Arial", 10, "bold")).grid(
        ...
        meta_frame = tk.LabelFrame(
            form_frame, text="ℹ️ Thông tin khác", font=("Arial", 10, "bold"), padx=10, pady=10
        )
        meta_frame.grid(row=7, column=0, columnspan=2, sticky="ew", pady=10)

        tk.Label(
            meta_frame,
            text=f"📊 Số embeddings: {current_info.get('embedding_count', 0)}",
            font=("Arial", 9),
        ).pack(anchor="w", pady=2)
        tk.Label(
            meta_frame,
            text=f"📅 Tạo lúc: {current_info.get('created_at', 'N/A')}",
            font=("Arial", 9),
        ).pack(anchor="w", pady=2)
        tk.Label(
            meta_frame,
            text=f"🔄 Cập nhật: {current_info.get('updated_at', 'N/A')}",
            font=("Arial", 9),
        ).pack(anchor="w", pady=2)
        ...
        def save_changes():
            ...
            if result["success"]:
                messagebox.showinfo(
                    "Thành công", f"✅ Đã cập nhật thông tin cho '{folder_name}'!", parent=dialog
                )
                dialog.destroy()
                self.load_known_faces()
                self.manage_faces()
                ...
                messagebox.showerror(
                    "Lỗi", f"❌ {result.get('message', 'Lỗi không xác định')}", parent=dialog
                )
    ...
    def recognize_from_webcam(self):
        ...
        title = tk.Label(
            self.right_panel, text="📹 Nhận diện từ Webcam", font=("Arial", 14, "bold"), bg="white"
        )
        title.pack(pady=10)
        ...
        if self.is_capturing and self.video_capture.isOpened():
            ...
            if ret:
                ...
                if hasattr(self, "face_infos") and hasattr(self, "face_locations"):
                    ...
                    for (top, right, bottom, left), info_lines in zip(
                        self.face_locations, self.face_infos
                    ):
                        # Scale coordinates
                        top *= 4
                        right *= 4
                        ...
                        for i, info_line in enumerate(info_lines):
                            ...
                            draw.rectangle(
                                [
                                    (left, y_pos - 2),
                                    (left + text_width + 10, y_pos + text_height + 2),
                                ],
                                fill=(0, 255, 0),
                            )
    ...
    def reload_data(self):
        ...
        messagebox.showinfo(
            "Thành công", f"✅ Đã tải lại {len(self.known_face_encodings)} khuôn mặt"
        )
