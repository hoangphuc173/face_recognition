...
def view_all_people():
    ...
    db_folders = [f for f in os.listdir(db.db_root) if os.path.isdir(os.path.join(db.db_root, f))]
    ...
    for i, folder_name in enumerate(sorted(db_folders), 1):
        ...
        if info:
            ...
            table_data.append([i, folder_name, "❌ ERROR", "-", "-", "-", "-", 0])
...
def view_person_detail(folder_name: str):
    ...
    if os.path.exists(faces_folder):
        image_files = [
            f for f in os.listdir(faces_folder) if f.lower().endswith((".jpg", ".jpeg", ".png"))
        ]
